...
import './PO.scss';
import {
    fetchCheckAllCT, getCheckChungTu, getCapNhapSoPONhapNCC, editMaTaiSan, fetchCheckOneCT, checkXoaChungTu,
    fetchCheckAllLogChungTuPO, fetchCheckAllLogChungTuHopDong, checkPO, fetchAllLogPOPR, checkXoaCheckChungTu, fetchAllLogCheck
...
import ModalCheckLogCTPO from './ModalCheckLogCTPO';
import ModalCheckLogCTHD from './ModalCheckLogCTHD';
import ModalXacNhanXoa from './ModalXacNhanXoa';
import ModalLogPOPR from './ModalLogPOPR';
import ModalCheckChungTu from "./ModalCheckChungTu";

const CheckNhapNhaCungCap = () => {
    ...
    const [rowDataLogCheck, setDataLogCheck] = useState();
    ...
    const [showModalLogCheck, setShowModalLogCheck] = useState(false);
    ...
    const [actionViewLogCheck, setActionViewLogCheck] = useState('VLOGCHECK');
    ...
    const handleCloseCheckXoaChungTu = () => {
        ...
        try {
            if (!machungtu.trim()) {
                ...
                if (res_action && res_action.errCode === 0) {
                    let res = await getCheckChungTu(machungtu, UserId);
    ...
    const handleXemLog = async (data) => {
        ...
        try {
            ...
            if (res_action && res_action.errCode === 0) {
                let response = await fetchCheckAllLogChungTuPO(sopo);
    ...
    const handleXemLogCTHD = async (data) => {
        ...
        try {
            ...
            if (res_action && res_action.errCode === 0) {
                let response = await fetchCheckAllLogChungTuHopDong(nhacungcap_id);
                if (response && response.errCode === 0) {
    ...
    const handleXemLogKiemTra = async (data) => {
        let machungtu = (data.data.machungtu)
        try {
            let res_action = await handleGetAction(actionViewLogCheck);
            if (res_action && res_action.errCode === 0) {
                let response = await fetchAllLogCheck(machungtu);
                if (response && response.errCode === 0) {
                    setDataLogCheck(response.data);
                    setShowModalLogCheck(true);
                }
            }
        } catch (e) {
            console.log(e);
    ...
    const handleXoaCheckChungTu = async (data) => {
        let machungtu = (data.data.machungtu)
        let sopo = (data.data.sopo)
        try {
            let res_action = await handleGetAction(actionXoaChungTu);
            if (res_action && res_action.errCode === 0) {
                let checkchungtu = await checkXoaCheckChungTu(machungtu);
                //console.log('checkchungtu', checkchungtu.errCode)
                if (checkchungtu && checkchungtu.errCode === 0) {
                    const modifiedData = { machungtu: machungtu, sopo: sopo, Nhom: 'xoacheckct' };
                    setDataXacNhanXoa(modifiedData);
                    setShowModalXacNhanXoa(true);
                } else {
    ...
    const [columnDefs, setColumnDefs] = useState([
                ...
                if (params.data) {
                    ...
                    return (
                        <div className="btn-action">
                            ...
                            <button className='btn-view-ncc px-1'
                                //disabled={trangthaihd === null} 
                                onClick={() => handleXemLogCTHD(params)}
                                data-tooltip-id="viewloghd">
                                <i
                                    style={{
                                        fontSize: '13px',
                                        fontStyle: 'normal',
                            ...
                            <button className='btn-delete' onClick={() => handleXoaCheckChungTu(params)} data-tooltip-id="delete"><i className="fas fa-trash"></i></button>
                        </div>
                    );
                }
                return null;
        ...
        },
        {
            ...
            maxWidth: 105,
            cellRenderer: (params) => {
                if (params.data && params.data.checkpo === 1) {
                    return (
                        <div style={{ textAlign: 'center' }}>
                            <button className='btn-view-ncc px-1' onClick={() => handleXemLogKiemTra(params)} data-tooltip-id="viewlogkiemtra">
                                <i
                                    style={{
                                        fontSize: '13px',
                                        fontStyle: 'normal',
                ...
                } else if (params.data && params.data.checkpo === 0) {
                    return (
                        <div style={{ textAlign: 'center' }}>
                            <button className='btn-view-ncc px-1' onClick={() => handleXemLogKiemTra(params)} data-tooltip-id="viewlogkiemtra">
                                <i
    ...
    const onFetchAllCT = useCallback(async () => {
        try {
            ...
            let response = await fetchCheckAllCT();
            if (response && response.errCode === 0) {
    ...
    const onFetchOneCT = useCallback(async () => {
        try {
            ...
            let response = await fetchCheckOneCT(machungtu);
            if (response && response.errCode === 0) {
    ...
    return (
        <>
            ...
            <div className='row grid-container'>
                ...
                <div>
                    <ModalCheckChungTu
                        show={showModalLogCheck}
                        rowDataLogCheck={rowDataLogCheck}
                        handleClose={handleClose}
                        onFetchAllCT={onFetchAllCT}
                        socket={socket}
                    />
                ...
                <div>
                    <ModalXacNhanXoa
                        ...
                        socket={socket}
                        handleCloseCheckXoaChungTu={handleCloseCheckXoaChungTu}
                    />
                </div>
                <div>
                    <ModalLogPOPR
...
export default CheckNhapNhaCungCap;